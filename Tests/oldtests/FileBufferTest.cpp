...
TEST(FileBuffer, interleaved_read_and_write) {
  ...
  while (numInsertsLeft > 0) {
    ...
    fb1.write(data1InPtr + cycle * numInsertsPerCycle * sizeof(int),
              curNumInserts * sizeof(int),
              CPU_BUFFER,
              cycle * numInsertsPerCycle * sizeof(int));
    ASSERT_EQ((cycle + 1) * numInsertsPerCycle * sizeof(int), fb1.size());
    fb2.write(data2InPtr + cycle * numInsertsPerCycle * sizeof(int),
              curNumInserts * sizeof(int),
              CPU_BUFFER,
              cycle * numInsertsPerCycle * sizeof(int));
