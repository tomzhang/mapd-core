...
class BaseScale {
    public:
        ...
        std::string getDomainGLSLUniformName() {
            return "uDomains_" + name;
        }

        std::string getRangeGLSLUniformName() {
            return "uRanges_" + name;
        }
...
class ScaleDomainRangeData {
    public:
        ...
        ScaleDomainRangeData(const std::string& name, bool useString = false) : _name(name), _useString(useString), _vectorPtr(nullptr) {}
        ~ScaleDomainRangeData() {}

        void initializeFromJSONObj(const rapidjson::Value& obj, const QueryRendererContextShPtr& ctx, BaseScale::ScaleType type);
        ...
        int size() {
            return (_vectorPtr == nullptr ? 0 : _vectorPtr->size());
        }
...
class BaseRenderProperty {
    public:
        BaseRenderProperty(const std::string& name, const QueryRendererContextShPtr& ctx, bool useScale = true) : _name(name), _useScale(useScale), _vboAttrName(""), _vboPtr(nullptr), _ctx(ctx), _inType(nullptr), _outType(nullptr), _scaleConfigPtr(nullptr) {};
        ...
        std::string getGLSLFunc() const {
            return "get" + _name;
        }

        std::string getInGLSLName() const {
            return "inT" + _name;
        }

        std::string getInGLSLType() const;

        std::string getOutGLSLName() const {
            return "outT" + _name;
        }

        std::string getOutGLSLType() const;

        bool usesScaleConfig() {
            return (_scaleConfigPtr != nullptr);
        }

        ScaleShPtr& getScaleConfig() {
            return _scaleConfigPtr;
        }
