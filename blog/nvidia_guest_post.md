...
Literals are packed into a byte buffer we pass as a parameter to the query. Instead of constants, we simply generate loads from that buffer and the loop invariant code motion in LLVM moves them before the loop and places them in registers. An example of what happens for the `SELECT COUNT(*) FROM test WHERE x > 5` query is shown below.

```
; The loop body, %13 contains the current x.
...
%14 = bitcast i8* %literals to i32*
%15 = load i32* %14
...
...
```
; The constant load moves before the loop.
...
%9 = bitcast i8* %literals to i32*
%10 = load i32* %9
...
; The loop body, %16 contains the current x.
...
%19 = icmp sgt i32 %16, %10
...
```

Note that, unlike LLVM IR, unaligned loads are not allowed in NVVM IR. The address of a load must be a multiple of the size of the type; otherwise, the query would crash with an invalid memory access error on the GPU, even if the load is not annotated as aligned. This can be easily fixed by adding padding bytes when we serialize the buffer.
...
The UI of our product is based on [Crossfilter](https://github.com/square/crossfilter) and allows user interaction with coordinated views. Clicking on a group or selecting a time window applies the filter to all other charts, which enables intuitive and highly interactive multivariate analysis. See the screenshot below for an example dashboard.

![Crossfilter UI](./crossfilter.png)

In the image, the user has just clicked on an airline in the top middle chart; the other five charts observe this action, only displaying the results for the selected airline.

 Since all the charts have to apply the same filter, there is a great opportunity to optimize the group of queries required to render the dashboard as opposed to each query in isolation. More specifically, the columns involved in the filter can only be scanned once by all the queries in the group, which is great for memory bandwidth usage. Also, LLVM optimization passes can benefit from the increased compilation scope of the combined query (for example, common sub-expressions can be computed only once).

We could also introduce adaptive strategies for group by evaluation. In practice, it's often the case that a relatively small subset of groups dominate the results -- think about the most engaged users of a social network or the most active gamers. We can learn about them in a probing phase and make sure they hash to a small memory range, minimizing write divergence. Granted, such a strategy is highly data dependent and we'll have to keep the penalty for being wrong under control, but the potential upside is very attractive.
