...
class ColorRGBA {
    public:
        ...
        ColorRGBA(const std::string& colorStr) {
            initFromCSSString(colorStr);
        }
        ColorRGBA(uint8_t r, uint8_t g, uint8_t b) : _colorArray({ {convertToColorChannel(r), convertToColorChannel(g), convertToColorChannel(b), 1.0} }) {}
        ColorRGBA(uint8_t r, uint8_t g, uint8_t b, uint8_t a) : _colorArray({ {convertToColorChannel(r), convertToColorChannel(g), convertToColorChannel(b), convertToColorChannel(a)} }) {}
