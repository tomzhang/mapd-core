...
QSConnection::QSConnection(IEnvironment* in_environment)
    : DSIConnection(in_environment), m_log(new DSIFileLogger("mapd_conn.log")), m_isConnected(false) {
...
// registry key on the server.
  ...
  m_settings.m_dbfPath = GetRequiredSetting(QS_DBF_KEY, in_connectionSettings).GetStringValue();

  simba_wstring user = GetRequiredSetting(QS_U_KEY, in_connectionSettings).GetStringValue();
  simba_wstring pass = GetRequiredSetting(QS_P_KEY, in_connectionSettings).GetStringValue();
  if (user != "a" || pass != "b") {
    QSTHROW2(DIAG_INVALID_AUTH_SPEC, L"QSUserPassError", user, pass);
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
bool QSConnection::PromptDialog(DSIConnSettingResponseMap& io_connResponseMap,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void QSConnection::UpdateConnectionSettings(const DSIConnSettingRequestMap& in_connectionSettings,
...
// If QuickstartDSII is built as a server, delegate looking for the DBF key to Connect().
  ...
  VerifyRequiredSetting(QS_U_KEY, in_connectionSettings, out_connectionSettings);
  VerifyRequiredSetting(QS_P_KEY, in_connectionSettings, out_connectionSettings);
